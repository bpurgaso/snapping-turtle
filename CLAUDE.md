...
client-linux/  Rust crate (the one non-TypeScript component): src/capture/ (portal via ashpd, KWin ScreenShot2 via zbus, QImage→PNG), tray (ksni), global shortcuts, IPC single instance, keyring (oo7) / 0600 file, upload; packaging/ (desktop file with the KWin annotation, RPM spec, icons); scripts/ (package-rpm.sh, integration.sh); README.md per-mode findings; TESTING.md manual checklist.
...
(cd client-linux && cargo fmt --check && cargo clippy --all-targets --locked -- -D warnings && cargo test --locked && cargo build --release --locked)   # the Linux client's contract (CI runs it in fedora:44 on the toolchain rust-toolchain.toml pins)
...
client-linux/scripts/package-rpm.sh          # release binary + RPM into client-linux/dist (CLIENT_APP_ID, PUBLIC_HOST+PORT from deploy/.env)
DATABASE_URL=… client-linux/scripts/integration.sh   # real server + client upload + row/page assertions (runs in CI)
...
- **The one language exception is `client-linux/` (Rust, M9).** Rationale: ashpd is the maintained first-class XDG portal binding, the output is a single dependency-light binary with no runtime to install, and the client shares nothing with the renderer code where the shared-types rule matters. It mirrors the few constants it needs (`MAX_UPLOAD_MB`, image caps, field names, the secret-prefix length) and `cargo test` reads `shared/src/constants.ts` to prove they have not drifted (`client-linux/src/contract.rs`); the upload contract is read from the server code and tests, never assumed. Rules 3 and 12 apply unchanged: the token and full capture URLs never reach stderr, notifications or the config file. Rust dependencies carry the same one-line justification as npm ones (in `Cargo.toml`).
...
- `CLIENT_APP_ID` (deploy/.env) is the Linux client's reverse-DNS app id: the desktop file's name, the DBus name, what the client registers with the portal. Every remembered portal grant is keyed on it, so like `EXTENSION_GECKO_ID` it is pinned once and never changes; `package-rpm.sh` refuses a binary whose baked id disagrees.
- KWin authorises `org.kde.KWin.ScreenShot2` by executable path: the installed desktop file's `Exec` must be the real binary path and carry `X-KDE-DBUS-Restricted-Interfaces`. A binary anywhere else is refused (`NoAuthorized`) and the client silently uses the portal — so a development build needs a desktop file in `~/.local/share/applications` naming it (`client-linux/README.md`).
- `client-linux/rust-toolchain.toml` is the one compiler-version authority, `Cargo.lock` is committed, and every cargo invocation in CI carries `--locked`. Never install the compiler with `dnf` (CI or a container rehearsal): Fedora's `rust` is whatever its updates repo holds that day, clippy grows lints every release, and `-D warnings` turns each new lint into a failure — CI on 2026-09-04 ran clippy 1.98 against code the local 1.93 had passed. `scripts/toolchain.sh` is the install path everywhere; a deliberate bump edits the pin, fixes what the new clippy says, and keeps `rust-version` in `Cargo.toml` at or below it. This is the second platform-calibration lesson: the E1 parity goldens taught macOS-vs-Linux (Chromium hinting, `web/playwright.config.ts`), this taught local-vs-Actions — a green run on this machine is necessary and not sufficient; the proof is a watched green run on GitHub.
- The Plasma portal backend saves every portal screenshot to `~/Pictures/Screenshot_<timestamp>.png`; the client removes exactly the returned path when it is a fresh regular `.png` and never anything else. `source_url` is NULL for desktop captures — the capture page omits the source link and nothing else; do not turn the absence into a different response shape (§7).
- The crop (E4) is a viewport, never an edit: shapes and the crop are both in original-image pixels, `annotationSizes()` takes `effectiveWidth(image, crop)` in *both* renderers, and the flat route's fast path is "zero shapes **and** no crop" — a cropped, unannotated capture must render or viewers get the full image (§10). The overlay's `viewBox` is the crop rect and sharp's `extract` is the same rect; keep the two in step, and keep the parity crop fixtures when touching either.
