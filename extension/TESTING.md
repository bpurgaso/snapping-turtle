...
| 3.3a | Shortcut path with OS notifications blocked (macOS Focus / browser muted): the failure still shows — a red `!` badge appears on the toolbar icon (hover shows the message); opening the popup shows "Last capture failed just now: …" once and clears the badge. A later successful capture also clears it           | fixed 2026-08-30 (Playwright covers badge + popup); in-browser: unverified | fixed 2026-08-30; in-browser: unverified          |
