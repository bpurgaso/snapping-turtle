...
Default shortcut `Alt+Shift+F`. **Both browsers scroll and stitch** with
~600 ms between tiles, so a long page takes a visible while and the toolbar
badge shows a percentage. Firefox's native one-call `captureTab` exists only
for manifests that declare `<all_urls>`, which this one never does (E5,
PLAN.md §15/§17): the 0.1.0 Firefox build failed every full-page capture with
"captureTab is not a function" because row 6.1's Firefox column below had
never been run. The strategy is now feature-detected at runtime
(`src/lib/full-page-strategy.ts`), so Firefox behaves exactly like Chrome in
this section — the "Chrome" wording in the checks applies to both — and the
2026-09-07 headless run (`docs/firefox-capturetab-probe.md`, "End to end")
drove the built Firefox artifact through the shortcut to an uploaded capture.
...
| #    | Check                                                                                                                                                                                                                                                                                                        | Chrome                                                                                      | Firefox                                                                                                                                                        |
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 6.1  | **Long page** (≥ 10 screens): click **Full page** → popup closes; page scrolls to the top and steps down, badge counts `3%…100%`; then a tab opens with the whole page, no gaps, no repeated bands, no duplicated strip at the bottom                                                                        | stitch geometry: verified (unit); pipeline to gesture: verified (smoke); pixels: unverified | **failed on 0.1.0 (the E5 bug)**; 0.1.1: stitched end to end headless on Firefox 154 (shortcut → 1000×5000 upload, 2026-09-07); pixels + human run: unverified |
| 6.2  | Afterwards the page is scrolled back to where it was                                                                                                                                                                                                                                                         | verified (Playwright driver + smoke)                                                        | unverified (stitches since 0.1.1)                                                                                                                              |
| 6.3  | **Sticky/fixed header page**: the header appears **once** at the top of the image, not in every tile; after capture the header is visible again on the page                                                                                                                                                  | hide/restore: verified (Playwright); pixels: unverified                                     | unverified — same hide/restore driver as Chrome since 0.1.1                                                                                                    |
...
| 6.6  | **Cancel mid-capture**: press Esc on the page during the scroll → capture stops, notification "Full-page capture cancelled.", `!` badge, page scrolled back, header visible                                                                                                                                  | cancel path: verified (unit); live: unverified                                              | unverified (stitches since 0.1.1)                                                                                                                              |
| 6.7  | **Tab switch mid-capture**: switch tabs during the scroll → notification "Capture stopped: the tab was switched away mid-capture.", original tab restored                                                                                                                                                    | abort path: verified (unit); live: unverified                                               | unverified (stitches since 0.1.1)                                                                                                                              |
...
| 6.12 | Firefox only: `about:debugging` → Inspect → console shows no "captureTab rendered at …×" line (would mean the browser ignored `scale`; the capture is then cropped to the cap, not wrong, but tell PLAN.md)                                                                                                  | n/a                                                                                         | n/a since 0.1.1 — the native path does not run without `<all_urls>` (E5)                                                                                       |
...
| 6.14 | **Failure wording** (E5): make a full-page capture fail (e.g. stop the server first) → the notification and the popup's "Last capture failed" line say what failed and suggest Visible or Region; no minified name (`f.default…`), no "is not a function", no stack — those go to the extension console only | unit: verified (`failure-text.test.ts`); live: unverified                                   | unit: verified; live: unverified                                                                                                                               |
