...
**Upload + errors:** the produced PNG posts to `${server}/api/v1/captures` (multipart, fields from `shared/` `CAPTURE_UPLOAD_FIELDS`) with the bearer token and `{sourceUrl: tab.url, title: tab.title}`, `credentials: 'omit'` and no redirect following; the returned `pageUrl` (accepted only if it is an http(s) URL) opens in a new tab next to the source. Failures surface as a notification; a 401 — or no token configured yet — opens the options page. The token is read from `storage.local` for the request and never logged or echoed in any message.
