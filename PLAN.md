...
- **Supply chain in CI.** `pnpm audit --audit-level=high` (reviewed ignores in `package.json` `pnpm.auditConfig`, reasons in `docs/supply-chain.md`), Trivy on the built app, backup and Caddy images (HIGH/CRITICAL with a fix fail; `deploy/.trivyignore` for reviewed exceptions), and grouped weekly Dependabot with majors of Fabric.js/sharp/argon2/Fastify excluded from automation. The images upgrade their base OS at build time; the app runtime drops npm/corepack/yarn, the backup image drops `gosu`, and Caddy is built from an exact patch release with Go modules bumped where a release still pins vulnerable ones.
