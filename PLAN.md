...
- **"Anonymous"** for the general cap means no *validly signed* session cookie and no bearer header — the signature check is cheap and unforgeable, and routes still verify credentials for real. Denied requests are not recorded, so windows are memory-bounded; distinct-key maps are capped with oldest-activity eviction.
