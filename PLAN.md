...
**Permissions (deliberately minimal):** `activeTab` (grants capture and injection rights on the invoked tab without `<all_urls>`), `scripting` (injects the region-select overlay and the full-page scroll driver), `storage`, plus a `host_permissions` entry for the build-time default server and `optional_host_permissions: ["https://*/*"]` so pointing the extension at a new domain triggers a one-time permission prompt for just that origin. Restricted pages (`chrome://`, `about:`, the extension stores) can't be captured or injected; the extension detects this and tells the user instead of failing silently.

**Mode selection:** the toolbar button opens a three-button popup — Visible / Region / Full page — and each mode also gets its own configurable keyboard command. The popup highlights the last-used mode.

**Visible viewport:** `captureVisibleTab({format:"png"})` → blob → upload. This is the M2 walking skeleton.
...
**Upload + errors:** the produced PNG posts to `${server}/api/v1/captures` with the bearer token and `{sourceUrl: tab.url, title: tab.title}`, and the returned `pageUrl` opens in a new tab. Failures surface as a notification; a 401 opens the options page.

**Options page:** server origin (pre-filled from `PUBLIC_ORIGIN` at build time), API token (stored in `storage.local`, deliberately *not* `storage.sync`, so the secret never transits sync infrastructure), and a "test connection" button.
...
| M8 | Store packaging (Web Store + AMO-signed xpi), domain-migration runbook, README | 2–3 d |
