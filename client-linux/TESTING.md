...
| 2 | Launch **snapping-turtle** from Kickoff | Tray icon appears in the system tray; tooltip "Ready"; Plasma's global-shortcut grants dialog appears once for `capture-full/region/window` | verified 2026-09-03 from a terminal launch (StatusNotifierItem registered, grants dialog shown and approved); Kickoff launch unverified |
...
| 4 | Tray → **Capture window…** | KWin's crosshair and on-screen hint; click a window → that window (decorated, no shadow) uploads; Escape cancels with no upload and no error notification | verified 2026-09-03 via `--capture window` (KWin `CaptureInteractive(window)`, 1936×1273 after a click, uploaded); Escape path and tray entry unverified |
| 5 | Tray → **Capture region…** | The Plasma portal chooser (Full Screen / Current Screen / Active Window, cursor and border toggles, preview, Share); Share uploads; Cancel → nothing. Note the absence of a rectangle option — that is the measured Plasma 6.7 limitation (README) | verified 2026-09-03 via a forwarded `--capture region` (chooser shown, Share → `~/Pictures/Screenshot_<ts>.png` uploaded and removed, page opened); Cancel path and tray entry unverified |
...
| 7 | Portal permission persistence: run the binary from a path KWin does not authorise (`cp /usr/bin/snapping-turtle /tmp/st && /tmp/st --capture full`) | First run: "Allow snapping-turtle to Take Screenshots?" — Allow; the whole workspace uploads. Second run: no prompt. The remembered grant is per app id (`flatpak permission-show screenshot` lists it, or the permission store table `screenshot`). A Deny is remembered the same way (the client says so and how to reset it) | verified 2026-09-03: Deny → remembered "no" and a clear failure; after reset, Allow → `~/Pictures/Screenshot_<ts>.png` written, uploaded, removed; second run silent and immediate, permission store `yes` |
| 8 | Desktop actions: right-click the launcher/task-manager entry → each of the three | Same three flows, routed to the resident instance when it runs (one tray icon, one shortcut session) | routing verified 2026-09-03 (`--capture region` from a second process was forwarded to the resident, which captured and uploaded; a second resident launch reports "already running" and the first keeps the bus name); the launcher menu itself unverified |
| 9 | Global shortcuts: Meta+Alt+S / R / W (or what you granted) | Same three flows from any application. If the grant dialog left them unassigned (the client logs "(unassigned)"), assign them under System Settings → Keyboard → Shortcuts → snapping-turtle, where Meta+Alt+S/R/W are the recorded defaults | partly verified 2026-09-03: bind accepted, app registered in kglobalshortcutsrc with the three defaults, keys left unassigned by the dialog; activation unverified |
...
| 14 | Quit from the tray; `snapping-turtle --capture window` from a terminal | Works inline without the resident instance | inline window capture verified 2026-09-03; Quit verified over DBus (`org.snappingturtle.Control1.Quit`: process exits, tray item gone), the tray's Quit entry unverified |
...
| 16 | **Region via crop (E4):** tray → *Capture full screen*; on the capture page press **Crop**, drag a rectangle around the part you wanted, **Apply crop**; then open the page link and the image link in a private window and paste the page link in a Discord channel | The editor keeps showing the whole screen with everything outside the rectangle dimmed and the crop re-adjustable; the private-window page and the image link show only the rectangle at its own pixel size (page footer and PNG dimensions agree, strokes drawn at the crop's width); the Discord unfurl shows the cropped image at the cropped dimensions, not the full screen | unverified |
