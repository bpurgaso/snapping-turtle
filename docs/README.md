...
| [firefox-capturetab-probe.md](firefox-capturetab-probe.md) | M6, E5    | How does Firefox scale and clamp `captureTab` rects? (Output = rect × scale; no clamp; we enforce the cap.) Which permission exposes it? (`<all_urls>` only) |
